import base64, tempfile, torch, runpod
...
def handler(event):
    ...
    w = int(inp.get("width", 768)); h = int(inp.get("height", 512))
    nf = int(inp.get("num_frames", 97)); fps = int(inp.get("fps", 24))
    steps = int(inp.get("num_inference_steps", 30))
    seed = inp.get("seed")
    pipe = load()
    gen = torch.Generator(device="cuda").manual_seed(int(seed)) if seed not in (None, "") else None
