    ...
    return {
        ...
        "config": {
            ...
            "process": [{
                ...
                "save": {"dtype": "float16", "save_every": max(steps, 100000),
                ...
                "sample": {"sampler": "flowmatch", "sample_every": max(steps, 100000),
...
def handler(event):
    try:
        ...
        images = inp.get("images") or []          # list of base64 (data-url or raw)
        ...
        # sample images produced by ai-toolkit
        samples = glob.glob(os.path.join(out_dir, name, "samples", "*.jpg")) + \
                  glob.glob(os.path.join(out_dir, "**", "*.png"), recursive=True)
        sample_b64 = None
        if samples:
            try:
                with open(sorted(samples)[-1], "rb") as f:
                    sample_b64 = "data:image/png;base64," + base64.b64encode(f.read()).decode()
            except Exception:
                pass
        ...
        return {"error": "training timeout", "stdout_tail": (e.stdout or b"")[-2000:].decode(errors="ignore") if isinstance(e.stdout, bytes) else str(e.stdout)[-2000:]}
